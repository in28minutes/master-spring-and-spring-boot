...
- Filtering
  ...
  - http://localhost:8080/filtering-with-view
  - http://localhost:8080/filtering-list-with-view
- Actuator
...
### Spring Boot 4 Update – Dependency Modularization

Spring Boot 4 brings in modularization of starters. What earlier mapped to `spring-boot-starter-web` now maps to `spring-boot-starter-webmvc`, while the core functionality remains the same.

Another important change is in testing—rather than having a single `spring-boot-starter-test`, Spring Boot 4 provides starter-specific test dependencies such as `spring-boot-starter-webmvc-test` and `spring-boot-starter-data-jpa-test`.

These changes are part of an initiative to reduce dependency size and improve startup time.

- Going forward, older starters can be mentally mapped to their modular equivalents.
- The intent is to include only what is needed and keep applications as lean as possible.
...
Note: Using plural nouns in REST resource URLs (for example, /users, /posts) is a common industry practice and improves readability and clarity.
...
Note: You can download and use Talend API Tester extension
...
- Step 21 - 02 - Versioning using Spring Boot Versioning
